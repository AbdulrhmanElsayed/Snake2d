...
rand_x = random.randrange(block_size*2, display_width-block_size*3)
rand_y = random.randrange(block_size*2, display_height-block_size*3)
snake_list = []
snake_len = 1
gameExit = False
gameOver = False
...
def score_message(msg, color):
    screen_text = font.render(msg, True, color)
    gameDisplay.blit(screen_text, [display_width / 2 - 150, display_height / 2+100 ])
...
def game_intro():
    ...
    while intro:
        ...
        gameDisplay.blit(background, [-300, -200])   ############مهم
...
def game_loop():
    ...
    while not gameExit:
        ...
        if apples%5 == 0:
           ...
           # pygame.draw.rect(gameDisplay, red, [rand_x, rand_y, block_size+20, block_size+20])
        else:
            gameDisplay.blit(apple, [rand_x, rand_y])
          # pygame.draw.rect(gameDisplay, red, [rand_x, rand_y, block_size, block_size])
        snake_head = []
